...
func (s *Server) handleTransferRequest(req network.TransferRequest) network.TransferAck {
	...
	if req.ToServer != "" && req.ToServer != s.cfg.Server.ID {
		ack.Accepted = false
		ack.Message = "incorrect destination server"
		return ack
	}
	ack.Accepted = true
	ack.Message = "accepted"
	s.logger.Printf("transfer request entity %s from %s targeting chunk(%d,%d) reason=%s", req.EntityID, req.FromServer, req.GlobalChunkX, req.GlobalChunkY, req.Reason)
	return ack
}
